...
public:
    ...
    void union_(int p, int q)
    {
        ...
        {
            // 小树接在大树下，改小树的根为大树的根
            id[root_p] = root_q; // q放到p的根下面
            sz[root_q] += sz[root_p];
        }
        else
        {
            id[root_q] = root_p; // q放到p的根下面
            sz[root_p] += sz[root_q];
        }
        components_counts--;
...
void quick_union_test_v3()
{
    ...
    uf.union_(4,1);uf.print_tree();
    uf.union_(2,3);uf.print_tree();
    uf.union_(4,5);uf.print_tree();
    uf.union_(3,6);uf.print_tree();uf.printf_id();
